...
(
  set -e
  curl_test localhost:18000/put -H "Host:app.domain.test" -X PUT -d '{"chonkyegressenvoy":123}'
  assert_header "200 OK"
  assert_response "X-Emissary-Auth"
  assert_response "bearer"
  ...
  assert_response "chonkyegressenvoy"
)
end_test
...
(
  set -e
  curl_test localhost:18000/get -H "x-emissary-mode:asdfadf" -H "Host:app.domain.test"
  assert_header "200 OK"
  assert_response "X-Emissary-Auth"
  assert_response "bearer"
)
...
(
  set -e
  curl_test localhost:18000/get -H "Host:blah"
...
(
  set -e
  curl_test localhost:18000/get -H "X-Emissary-Auth:bearer fakeheader"
  assert_header "403 Forbidden"
)
end_test
...
(
  set -e
  curl_test localhost:8080/get -H "X-Emissary-Auth:blah"
  assert_header "403 Forbidden"
...
(
  set -e
  curl_test localhost:8080/patch -X PATCH -H "X-Emissary-Auth:bearer $jwt" -d '{"test":123}'
  assert_header "200 OK"
  assert_response "$jwt"
  assert_response '"X-Emissary-Auth-Status": "success"'
...
(
  set -e
  curl_test localhost:8080/patch -H "X-Emissary-Auth:bearer $jwt"
  assert_header "403 Forbidden"
  assert_header "x-emissary-auth-status: failure"
)
end_test
...
(
  set -e
  curl_test localhost:8080/put -X PUT -H "X-Emissary-Auth:bearer $jwt" -d '{"test":123}'
  assert_header "200 OK"
  assert_response "$jwt"
  assert_response '"X-Emissary-Auth-Status": "success"'
...
(
  set -e
  curl_test localhost:8080/nomatch -H "X-Emissary-Auth:bearer $jwt"
  assert_header "403 Forbidden"
  assert_header "x-emissary-auth-status: failure"
)
end_test
...
(
  set -e
  curl_test localhost:8080/get -X PUT -d '{}' -H "X-Emissary-Auth:bearer $jwt"
  assert_header "403 Forbidden"
  assert_header "x-emissary-auth-status: failure"
)
end_test
...
(
  set -e
  curl_test localhost:8080/put -H "X-Emissary-Auth:bearer $jwt" -X PUT -d '{"chonkyingressenvoy":123}'
  assert_header "200 OK"
  assert_response '"X-Emissary-Auth-Status": "success"'
  assert_response "$jwt"
  assert_response "chonkyingressenvoy"
...
(
  set -e
  curl_test localhost:8080/get -H "x-emissary-mode:asdfadf" -H "X-Emissary-Auth:bearer $jwt"
  assert_header "200 OK"
  assert_response "$jwt"
  assert_response '"X-Emissary-Auth-Status": "success"'
  docker-compose -f $DIR/../docker-compose.yaml logs emissary \
